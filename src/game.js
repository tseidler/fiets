// De race zelf: trappen met E/R, springen met spatie, obstakels ontwijken.
...
export class Race {
  constructor(renderer, char, bike, cb) {
    ...
    this.sun.shadow.camera.left = wide ? -45 : -30;
    this.sun.shadow.camera.right = wide ? 45 : 30;
    this.sun.shadow.camera.top = wide ? 45 : 35;
    this.sun.shadow.camera.bottom = wide ? -45 : -35;
    ...
    this.camPos = new THREE.Vector3(2.4, 3.1 + this.terrain.heightAt(this.z - 7.5), this.z - 7.5);
  ...
  onKey(e) {
    ...
    if (e.code === 'KeyR' || e.code === 'KeyU') {
      ...
      if (key !== this.lastKey) {
        // Goede afwisseling: volle pedaalslag
  ...
  update(dt) {
    ...
    if (this.crashed) {
      ...
      // Pitch-lerp (grijpt bewust NIET tijdens de crash-tuimel): op de grond
      // volgt de neus de helling, in de lucht de verticale snelheid.
      ...
      if (this.grounded) this.rig.rotation.z = 0;
    }
    ...
    this.rig.position.set(0, this.y, this.z);
    this.rig.userData.animate(dt, { phase: this.pedalPhase, speed: this.speed });

    // Camera volgt met lichte vertraging en snelheids-FOV; volgt h(z) zodat je
    // over toppen heen blijft kijken. Gehoiste vectors: geen allocaties.
    ...
    if (this.camMode === 1) {
      ...
      this._camTarget.set(0, 2.0 + this.terrain.heightAt(this.z - 3.4) + airLift * 0.5, this.z - 3.4);
      this._look.set(0, 1.15 + this.terrain.heightAt(this.z + 6), this.z + 6);
      ...
      this._camTarget.set(8, 2.1 + hHere + airLift * 0.4, this.z);
      this._look.set(0, 1.0 + hHere + airLift * 0.4, this.z + 3);
      ...
      this._look.set(0, 1.1 + this.terrain.heightAt(this.z + 9), this.z + 9);
    }
    ...
    // Zon-frustum blijft gecentreerd op de speler, meedeinend met h(z)
    this.sun.position.set(this.sunOff[0], this.sunOff[1] + hHere, this.z + this.sunOff[2]);
    this.sun.target.position.set(0, hHere, this.z + 10);
